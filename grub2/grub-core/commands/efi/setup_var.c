...
#define CMDNAME_SETUP_VAR2 ("setup_var_2")
#define CMDCHECK_SETUP_VAR2 (10)
				   ...
				   int argc, char *argv[])
{
	...
	if (cmd->name[CMDCHECK_SETUP_VAR2] != 0)
		isMode2 = 1;
	if (cmd->name[CMDCHECK_SETUP_VAR2] != 0 && cmd->name[CMDCHECK_SETUP_VAR3] != 0)
		isMode3 = 1;

	if (argc == 0)
	{
		grub_printf("Hello!\n");
		grub_printf("You may brick your InsydeH2o based laptop and need to send it in\n");
		grub_printf("for repair.\n");
		grub_printf("Some vendors (like Sony) are rather slow, and it could get expensive if they\n");
		grub_printf("discover that you used this or similar tools.\nYou should be  *very*  sure what you do.\n");
		grub_printf("\n\nThis Setup variable modification tool may only work with current (July 2009)\n");
		grub_printf("Insyde H2o based firmware releases. Vendors may decide to lock the EFI\n");
		grub_printf("environment completely out or introduce other security measures.\n");
		grub_printf("Final warning: YOU MAY BRICK YOUR LAPTOP IF YOU USE THIS TOOL - I TAKE  N O \n");
...
static grub_command_t cmd_setup_var_2;
...
GRUB_MOD_INIT(setup_var)
{
	cmd_setup_var = grub_register_command("setup_var", grub_cmd_setup_var,
										  N_("setup_var offset [setval]"),
										  N_("Read/Write specific (byte) offset of setup variable."));
	cmd_setup_var_2 = grub_register_command("setup_var_2", grub_cmd_setup_var,
											N_("setup_var_2 offset [setval]"),
											N_("Read/Write specific (byte) offset of setup and custom variables."));
	cmd_setup_var_3 = grub_register_command("setup_var_3", grub_cmd_setup_var,
											N_("setup_var_3 offset [setval]"),
											N_("Read/Write specific (byte) offset of setup variables ignoring error, use with great caution!!!"));
	cmd_setup_lsvar = grub_register_command("lsefivar", grub_cmd_lsefivar,
											N_("lsefivar"),
											N_("Lists all efi variables."));
}

GRUB_MOD_FINI(setup_var)
{
	grub_unregister_command(cmd_setup_var);
	grub_unregister_command(cmd_setup_var_2);
